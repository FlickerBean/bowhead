...
if (false === defined('TAVURTH_OANDAWRAP')) {
    ...
    class Oanda
    {
        ...
        public static function valid($jsonObject, $verbose = false, $message = false)
        {
            ...
            if (false === isset($jsonObject) || false === $jsonObject || empty($jsonObject)) {
                if ($verbose && $message) {
        ...
        protected static function setup_account($baseUrl, $apiKey = false, $accountId = false, $checkSSL = 2)
        {
            ...
            if (false !== strpos($baseUrl, 'https') || false !== strpos($baseUrl, 'fxpractice')) {
        ...
        private static function stream_callback($curl, $str)
        {
            ...
            if (null != ($decoded = @json_decode($str))) {
                // Call the users callback
                if (null != call_user_func(self::$callback, $decoded)) {
                    return false;
                }
        ...
        public static function stream($callback, $quotes = false, $accounts = false)
        {
            ...
            if (false !== $accounts && !empty($accounts)) {
        ...
        public static function nav_pnl($dollarValue = false)
        {
            ...
            if (0 == self::$account->balance) {
                return 0.00;
            }

            //Percentage
            if (false === $dollarValue) {
        ...
        public static function order_open($side, $units, $pair, $type, $price = false, $expiry = false, $rest = false)
        {
            ...
            if ('market' !== $type && (false === $price || false === $expiry)) {
                return false;
            }
        ...
        public static function position_pnl_pips($pair)
        {
            ...
            $price = 'buy' == $position->side ? self::price($pair)->bid : self::price($pair)->ask;
