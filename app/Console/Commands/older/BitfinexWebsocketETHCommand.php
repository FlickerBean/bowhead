...
class BitfinexWebsocketETHCommand extends Command
{
    ...
    public function manageCacheArray($key, $item, $len = 60)
    {
        ...
        if (\Cache::has($key)) {
            ...
            krsort($storeArr);
            if ('bitfinex::ticker::last_price_diff::array' == $key || 'bitfinex::ticker::last_price::array' == $key) {
                //print_r($storeArr);
            }
        ...
        $connector('wss://api.bitfinex.com/ws')
            ->then(function (\Ratchet\Client\WebSocket $conn) {
                ...
                $conn->on('message', function (\Ratchet\RFC6455\Messaging\MessageInterface $msg) use ($conn) {
                    ...
                    if ((!empty($data['event']) && 'subscribed' == $data['event']) ? true : false) {
                        if (!empty($data['channel'])) {
                            $this->updChannels($data['channel'], $data['chanId']);
                        }
                    }
                    if (!empty($data[0]) && 'hb' != $data[1]) {
